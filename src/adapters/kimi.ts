...
export class KimiAdapter implements AgentAdapter {
  readonly id = "kimi";
  readonly name = "Kimi (kimi CLI)";

  async healthCheck(): Promise<HealthResult> {
    return probeBinary("kimi", ["--version"], HEALTH_CHECK_TIMEOUT_MS);
  }
  ...
  async execute(req: AgentRequest): Promise<AgentResponse> {
    ...
    try {
      await withTimeout(proc.exited, timeout, "kimi");
    } catch (err) {
      controller.abort();
      await proc.exited;
    ...
    if (proc.exitCode !== 0) {
      ...
      return { content: "", agent: this.id, latency_ms: Date.now() - t0, error: `kimi exited ${proc.exitCode}: ${stderr.slice(0, 200)}` };
    }

    const content = await stdoutPromise;
    if (!content.trim()) {
      return { content: "", agent: this.id, latency_ms: Date.now() - t0, error: "kimi returned empty response" };
    }

    return {
      content: content.trim(),
      agent: this.id,
      model: AGENT_MODELS.kimi ?? "default",
